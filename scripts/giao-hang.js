...
(function () {
  ...
  function _ghOrders() { return S().get('orders', window.ORDERS || []) || []; }
  function _ghKg(o) { return +o.weight || (Array.isArray(o.items) ? o.items.reduce((s, it) => s + (+it.qty || 0), 0) : 0); }
  function _ghRealDriver(o) { const n = String(o.driverName || o.driver_name || '').trim(); return (n && n !== '—') ? n : ''; }
  const _q1 = n => { n = +n || 0; return (n % 1 ? Math.round(n * 10) / 10 : n).toLocaleString('vi-VN'); };
  let _ghStatsMonth = null;

  function _ghStatsData(monthISO) {
    const [y, m] = monthISO.split('-').map(Number);
    const last = new Date(y, m, 0).getDate();
    const byShip = {};
    _ghOrders().forEach(o => {
      if (o.status !== 'delivered' && o.status !== 'reconciled') return;
      const drv = _ghRealDriver(o); if (!drv) return;
  ...
  window.ghShipperStats = function () { _ghStatsMonth = (window.todayISO ? window.todayISO() : '2026-07').slice(0, 7); _ghRenderStats(); };
  window.ghStatsSetMonth = function (v) { if (v) { _ghStatsMonth = v; _ghRenderStats(); } };

  function _ghRenderStats() {
    const D = _ghStatsData(_ghStatsMonth);
    ...
    const rows = D.list.length ? D.list.map(s => `<tr>
      ...
      : `<tr><td colspan="${D.last + 3}" style="padding:34px;text-align:center;color:var(--muted)">Chưa có đơn giao nào gán shipper tháng này.<br><span style="font-size:12px">Gán shipper cho đơn → giao xong → thống kê tự lên.</span></td></tr>`;
    ...
    const body = `
      <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:10px">
        <label style="font-size:13px;color:var(--muted)">Tháng: <input type="month" value="${_ghStatsMonth}" onchange="window.ghStatsSetMonth(this.value)" style="padding:6px 9px;border:1px solid var(--line);border-radius:7px;font-size:13px"></label>
        <div style="flex:1"></div>
        <div style="font-size:12.5px;color:var(--muted)">Σ <b style="color:#15803D">${gO} đơn</b> · <b style="color:#B45309">${_q1(gKg)} kg</b> · ${D.list.length} shipper</div>
        <button class="btn btn-ghost" onclick="window.ghStatsImage()" title="Copy ảnh tóm tắt tháng">📸 Copy ảnh</button>
        <button class="btn btn-primary" onclick="window.ghStatsExcel()">📊 Xuất Excel</button>
      </div>
    ...
    window.openModal('📊 Thống kê hiệu suất Shipper', body, { width: '90vw' });
  }

  window.ghStatsExcel = function () {
    ...
    const days = Array.from({ length: D.last }, (_, i) => i + 1);
    const aoa = [['THỐNG KÊ HIỆU SUẤT SHIPPER · Tháng ' + _ghStatsMonth.slice(5) + '/' + _ghStatsMonth.slice(0, 4)],
    ...
    window.XLSX.writeFile(wb, 'HieuSuat-Shipper_' + _ghStatsMonth + '.xlsx');
    window.toast && window.toast('✓ Đã xuất Excel', 'success');
  };

  async function _ghLogoDataURL() {
  ...
  window.ghStatsImage = async function () {
    const D = _ghStatsData(_ghStatsMonth);
    if (!D.list.length) { window.toast && window.toast('Chưa có dữ liệu để copy', 'info'); return; }
    const logo = await _ghLogoDataURL();
    const mLabel = _ghStatsMonth.slice(5) + '/' + _ghStatsMonth.slice(0, 4);
    const gO = D.list.reduce((s, x) => s + x.totO, 0), gKg = D.list.reduce((s, x) => s + x.totKg, 0);
