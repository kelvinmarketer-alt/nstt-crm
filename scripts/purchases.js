      ...
      <table style="width:100%;border-collapse:collapse;font-size:13px;margin-bottom:12px">
        ...
        <tbody id="pnEditItems">
          ${(p.items||[]).map((it,i) => {
            ...
            const recvInfo = it.recvQty != null ? `<div style="font-size:10.5px;color:${it.defectQty ? '#B45309' : '#15803D'}">✓ thực nhận ${_q(it.recvQty)}${it.packRecv ? ` <span style="color:var(--muted)">(${_q(it.packRecv)} ${_escP(it.packUnit || 'đơn vị')})</span>` : ''}${it.defectQty ? ` · lỗi ${_q(it.defectQty)} (${it.canReturn ? 'trả NCC' : 'hao hụt'})` : ''}${it.stockedQty ? ` · dư vào kho ${_q(it.stockedQty)}` : ''}</div>` : '';
