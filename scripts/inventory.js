...
(function () {
  ...
  window.invApply = function (productId, deltaQty) {
    ...
    /* Cột inventory.stock đã là numeric → lưu kg lẻ (giữ 2 số thập phân, tránh nhiễu số thực). */
    const _r2 = n => Math.max(0, Math.round((+n || 0) * 100) / 100);
    if (!item) {
      ...
      window.STORE.add('inventory', {
        ...
        stock: _r2(deltaQty), minStock: 10, maxStock: 100, avgDaily: 5,
        lastIn: deltaQty > 0 ? vi : '', lastOut: deltaQty < 0 ? vi : '', location: 'Kho A1',
      });
      return;
    }
    const patch = { stock: _r2((item.stock || 0) + deltaQty) };
